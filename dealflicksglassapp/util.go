...
// func convertToOAuthToken(DFToken *dftoken) oauth.Token{
// 	return &oauth.Token{
// 		AccessToken: dftoken.AccessToken,
// 		RefreshToken: dftoken.RefreshToken,
// 		Expiry: dftoken.Expiry,
// 		Extra: nil
// 	}
// }
...
func storeCredential(c appengine.Context, userID string, token *oauth.Token) error {
	...
	_, err := datastore.Put(c, key, dftoken)
	return err
}
...
func authTransport(c appengine.Context, userID string) *oauth.Transport {
	...
	// tok := new(oauth.Token)
	dftok := new(DFToken)
	if err := datastore.Get(c, key, dftok); err != nil {
		c.Errorf("###Get Token: %v", err)
		return nil
	}
	tok := &oauth.Token{
		AccessToken: dftok.AccessToken,
		RefreshToken: dftok.RefreshToken,
		Expiry: dftok.Expiry,
		Extra: nil,
	}
