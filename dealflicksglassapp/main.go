...
func init() {
	...
	http.HandleFunc("/test", errorAdapter(testHandler))
...
func testHandler(w http.ResponseWriter, r *http.Request) error {
	// <img src="/attachmentproxy?attachment=ps%3a5944745894914767410&amp;timelineItem=f64f07cc-dd01-4351-9af0-5ea30c9396ec" width="150">
	itemId := "f64f07cc-dd01-4351-9af0-5ea30c9396ec"
	attachmentId := "ps:3a5944745894914767410"
	...
	k, _ := wrt.Key()
	c.Infof("####### %v", k)
	imageUrl := fmt.Sprintf("http://10.10.2.217:8080/serve/?blobKey=%s", k)
    ...
    resp, err = client.PostForm("http://10.10.2.208:9999/picture-search", url.Values{"imgurl": {imageUrl}})
    if err != nil {
...
func rootHandler(w http.ResponseWriter, r *http.Request) error {
	...
	contact, err := svc.Contacts.Get("Dealflicks Glass App").Do()
